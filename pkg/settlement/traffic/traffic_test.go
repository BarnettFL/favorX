...
func newP2pService(t *testing.T, networkID uint64, o libp2pServiceOpts) (s *libp2p.Service, overlay boson.Address) {
	...
	overlay, err = crypto.NewOverlayAddress(crypto.FromPublicKey(privKey.GetPublic()), networkID)
	if err != nil {
		t.Fatal(err)
	}
	...
	s, err = libp2p.New(ctx, crypto.NewDefaultSigner(privKey.IntoKey().(*ecdsa.PrivateKey)), networkID, overlay, addr, o.Addressbook, statestore, o.lightNodes, o.bootNodes, o.Logger, nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	s.Ready()
